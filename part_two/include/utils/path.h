...
inline priority_queue_asc<int> random_cut_points(const size_t parents_len) {
  if (parents_len < 2) {
    throw std::invalid_argument(
        "Path length must be at least 2 to generate cut points.");
  }
...
// Returns a priority queue of (index, fitness) pairs sorted according to the
// provided comparator.
inline priority_queue_by_fitness sort_by_fitness_idx(const vector<Chromosome> &chromosomes, const pair_comp &comparator) {
