...
vector<Chromosome>
OrderCrossover::recombine(const vector<Chromosome> &parents) {
  ...
  for (int i = 0; i < n_parents; ++i) {
    ...
    for (int j = 0; j < parents[i].get_n_genes(); ++j) {
      ...
      if (used_nodes.find(parents[1 - i].get_node(current_index).id) ==
          used_nodes.end()) {
        offsprings[i][(cut2 + 1 + j) % parents[i].get_n_genes()] =
            parents[1 - i].get_node(current_index);
      }
