...
vector<Chromosome>
SteadyStateReplacement::replace(const vector<Chromosome> &parents,
                                const vector<Chromosome> &offsprings) const {
  auto parents_idx_by_fitness = sort_by_fitness_idx(parents, by_fitness_desc);
  auto offsprings_idx_by_fitness =
      sort_by_fitness_idx(offsprings, by_fitness_desc);
  ...
  for (size_t i = 0; i < parents.size(); ++i) {
    if (i < to_be_replaced) {
      ...
      new_population.push_back(
          offsprings[offsprings_idx_by_fitness.top().first]);
